...
def start_simulating():
    ...
    num_particles = []
    robot = np.array([0.,0.])
    particles = np.random.normal(loc=0,scale=2, size=[INIT_NP, 2])
    ...
    return errors, elapse, num_particles
...
if __name__ == "__main__": 
    ...
    for t in [1]:
        ...
        while NUM_RUN < 500:
            ...
            errors, elapsed, num_particles = start_simulating()   
            COLLECTION['errors'].append(errors)
            COLLECTION['elapsed'].append(elapsed)
            COLLECTION['num_particles'].append(num_particles)
            NUM_RUN+=1
        ...
        with open(f'COLLETION_MREKLD_{INIT_NP}_{t}.pkl', 'wb') as sample_bak:
